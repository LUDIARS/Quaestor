...
これを `spec/data/account-codes.md` に切り出して Quaestor の標準科目テーブルとして同梱する (ユーザがカスタム編集可)。
...
- `教育機関入金レポート.md` `EducationPartner.md` のように 取引先固有の入金パターン (源泉徴収あり) も別建てメモがある。 Quaestor では「源泉付き売上」 entry type を別扱い
